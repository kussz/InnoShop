...
namespace InnoShop.Infrastructure.Repositories
{
    public class ProductRepository : RepositoryBase<Product>, IProductRepository
    {
        ...
        {
            var product = FindByCondition(u => u.Id == id).Include(p=>p.ProdType).Include(p=>p.User).Include(p=>p.Buyer).Include(p=>p.ProdAttribs).Single();
            return product;
        }
        ...
        public List<Product> GetPage(int quantity, int page,ProductFilterDTO dto)
        {
            if (dto.Search == null)
                dto.Search = "";
            var prods= Context.Set<Product>().OrderByDescending(p=>p.CreationDate).Include(p => p.ProdType).Where(p=>p.Public&&(dto.CategoryId!=0?dto.CategoryId==p.ProdTypeId:true)&&p.Cost>=dto.MinPrice&&p.Cost<=dto.MaxPrice&&p.BuyerId==null&&(p.Description.Contains(dto.Search)||p.Name.Contains(dto.Search)||p.ProdAttribs.Any(t=>t.Name.Contains(dto.Search)))).Skip(quantity * (page - 1)).Take(quantity).ToList();
