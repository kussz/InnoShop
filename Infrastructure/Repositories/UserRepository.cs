...
namespace InnoShop.Infrastructure.Repositories
{
    public class UserRepository : RepositoryBase<User>, IUserRepository
    {
        ...
        public User GetUser(int id)
        {
            ...
            //if (user == null)
            //{
                var user = FindByCondition(u => u.Id == id).Include(p=>p.Locality).Include(p=>p.UserType).Include(p=>p.ProductsUser).ThenInclude(p=>p.Buyer).Include(p => p.ProductsBuyer).Single();
